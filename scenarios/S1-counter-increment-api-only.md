...
S1 measures exactly that cost and tracks it release-over-release.

While this no-op requirement applies to all signals, S1 focuses on the metrics
API as the first scenario; equivalent API-only scenarios for traces and logs
are reasonable follow-up work. Concretely, S1 measures the cost of a single
...
- Workload. Increment a `Counter` instrument named `house.energy.consumed`
  by `1` in a tight loop, with the same attribute values on every call.
...
- Threading. Single-threaded (one OS thread). Async or coroutine-style
  code is fine where idiomatic. Multi-threaded and contended variants are out
  of scope for S1 and deferred to follow-up scenarios.
...
"API-only" maps to a specific package per language. Each harness pins and
records the version of that package against every data point.

- .NET — The metrics API ships in the BCL
